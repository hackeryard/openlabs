...
- **`middleware.ts`** (root — the only middleware Next.js loads; `app/middleware/middleware.js` is confirmed-dead code, unreferenced anywhere, targeting a nonexistent `/dashboard/:path*` matcher) checks *presence* and *expiration* of `auth-token`. Always-passthrough regardless of auth state: `/api/auth/*`, `/api/challenges/generate`, `/api/contact`, `/api/blogs`. Public without auth: `/`, `/login`, `/signup`, `/forgotpassword`, `/reset-password`, `/verify-email`, `/about`, `/contact`, plus prefixes `/blog`, `/physics`, `/chemistry`, `/biology`, `/computer-science` (the landing-page tier only). **Not public**: `/labs/*` (the actual interactive-lab tier) and `/admin/*` — both require authenticated accounts with appropriate roles.
