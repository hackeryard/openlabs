...
**OpenLabs** (`openlabs.org.in`) is a Next.js 14 (App Router) education platform offering free interactive virtual labs across **Physics, Chemistry, Biology, and Computer Science**, plus a gamification layer (XP/levels/streaks/badges/daily AI-generated challenges), an editorial blog, and a context-aware AI chat assistant. Proprietary license, single maintainer (`@rahulra3621`).
...
4. **Registry entry** — `app/lib/labs.ts`, the `LABS` array (52 entries as of this writing): `{ id, name, subject: "physics"|"chemistry"|"biology"|"computerScience", type: "simulation"|"exploration"|"editor", challengeParams: string[], challengeEnabled, description }`. `id` is the `<subject>/<lab-slug>` path segment used to key `DailyChallenge`/`Project`/XP records — **must match** the `labId` string passed to `useLab()`/`DailyChallengeCard` in the lab component. A lab isn't "wired up" for gamification until it's in here — see [Known drift](#known-drift--rough-edges) for labs that exist as routes but aren't registered.
...
- **`app/lib/labs.ts` registry drift from actual routes**: `physics/opticslens` has a working route (`app/physics/opticslens`, `app/labs/physics/opticslens`) but no `LABS` entry. Several `computer-science/ai-problem/*` subfolders are unregistered — only `hangman`, `hill-climb`, `maze-qlearn` are in `LABS`; `constraint-satisfy`, `forward-backward`, `monkey-banana`, `water-jug`, and `neural-network` (which is a `.jsx` page, the only non-`.tsx` page in that folder) have working routes but aren't registered, so they get no gamification/XP/daily-challenge support. Run the `audit-labs-registry` skill before trusting `LABS` as a complete lab list.
- **The Mathematics subject and the Java code lab were removed entirely** (July 2026) — both existed only as unfinished stubs (`/maths/alzebra` rendered the literal placeholder text "dfghjkl"; `/computer-science/code-lab/java`'s interactive page was an empty `<main>` with a commented-out `<Hero />`) that were reachable/crawlable with misleading SEO metadata promising working features. Don't re-add either without real content behind them — if resurrected, wire up `app/lib/labs.ts`'s `Lab["subject"]` union (mathematics was removed from it) and re-add the routes to `app/sitemap.ts`/`app/robots.ts`/`middleware.ts`'s public-prefix lists.
