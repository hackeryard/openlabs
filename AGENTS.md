...
- New interactive labs require **all** of: a landing page (`app/<subject>/<lab>/page.tsx`), a simulation route (`app/labs/<subject>/<lab>/page.tsx`, dynamic-imported with `ssr: false`), the component itself (`app/components/<subject>/<LabName>Lab.jsx`), a registry entry in `app/lib/labs.ts`, and a Navbar link (`app/components/Navbar.tsx`). See `CLAUDE.md` for the full pattern.
- `/labs/*` and `/admin/*` require auth (enforced in root `middleware.ts`); subject landing pages and `/blog` are public.
- Don't extend `app/api/agent`, `app/api/auth/run`, or `app/middleware/middleware.js` — they're dead/unimplemented code paths, not the active implementation (see `CLAUDE.md` "Known rough edges").
