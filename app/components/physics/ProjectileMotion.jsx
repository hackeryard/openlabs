...
export default function ProjectileMotionLab({
  ...
  function exportCSV(){
    const rows = [["t_s","x_m","y_m"], ...trajRef.current.map(p => [p.t.toFixed(4), p.x.toFixed(4), p.y.toFixed(4)])];
    const csv = rows.map(r=>r.join(",")).join("\n"); const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `projectile_${Date.now()}.csv`; a.click(); URL.revokeObjectURL(url);
  }
  ...
  function exportReport(){ const html=`<html><body><h1>Projectile Lab</h1><p>Date: ${new Date().toLocaleString()}</p><pre>v0=${v0} m/s, angle=${angle}°, h0=${h0} m, g=${g}</pre><p>TOF: ${tof||"—"}, Range: ${range||"—"}</p><button onclick="window.print()">Print</button></body></html>`; const w=window.open("","_blank"); if(!w) return alert("popup blocked"); w.document.write(html); w.document.close(); }

  return (
    <div className="max-w-6xl mx-auto p-4 space-y-4">
      <header className="flex items-center justify-between"><h2 className="text-2xl font-semibold">Projectile Motion — Virtual Lab</h2><div className="text-sm text-gray-600">Simulate launches, measure range & TOF</div></header>
