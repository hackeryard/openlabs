...
export default function MacrotaskQueue({ entries, isActive, isDimmed }: MacrotaskQueueProps) {
  ...
  return (
    ...
    `}>
      {/* Header */}
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-amber-200 dark:border-amber-500/20">
        <div className={`w-6 h-6 rounded-md bg-amber-100 dark:bg-amber-500/20 flex items-center justify-center ${isActive && !shouldReduceMotion ? 'animate-pulse' : ''}`}>
          <ClipboardList className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
        </div>
        <h3 className="text-sm font-semibold text-amber-700 dark:text-amber-300 tracking-wide">Macrotask Queue</h3>
        <span className="ml-auto text-xs font-mono text-amber-600/70 dark:text-amber-400/60">{entries.length}</span>
      </div>

      {/* Info */}
      <div className="px-3 py-1.5 bg-amber-50 dark:bg-amber-500/5 border-b border-amber-100 dark:border-amber-500/10">
        <p className="text-[10px] text-amber-700/80 dark:text-amber-400/70 font-medium">
          ...
          <AnimatePresence>
            ...
            {entries.map(entry => (
              <motion.div
                ...
                `}
              >
                <span className={`font-mono text-xs font-medium whitespace-nowrap ${isDimmed ? 'text-amber-700/50 dark:text-amber-300/50' : 'text-amber-700 dark:text-amber-300'}`}>
                  {entry.label}
                </span>
              </motion.div>
            ))}
