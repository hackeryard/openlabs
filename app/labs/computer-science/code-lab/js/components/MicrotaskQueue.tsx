...
export default function MicrotaskQueue({ entries, isActive }: MicrotaskQueueProps) {
  ...
  return (
    <div className={`flex flex-col h-full transition-shadow duration-500 ${isActive ? 'shadow-[0_0_20px_rgba(16,185,129,0.15)]' : ''}`}>
      {/* Header */}
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-emerald-200 dark:border-emerald-500/20">
        <div className={`w-6 h-6 rounded-md bg-emerald-100 dark:bg-emerald-500/20 flex items-center justify-center ${isActive && !shouldReduceMotion ? 'animate-pulse' : ''}`}>
          <Zap className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
        </div>
        <h3 className="text-sm font-semibold text-emerald-700 dark:text-emerald-300 tracking-wide">Microtask Queue</h3>
        <span className="ml-auto text-xs font-mono text-emerald-600/70 dark:text-emerald-400/60">{entries.length}</span>
      </div>

      {/* Priority notice */}
      <div className="px-3 py-1.5 bg-emerald-50 dark:bg-emerald-500/5 border-b border-emerald-100 dark:border-emerald-500/10">
        <p className="text-[10px] text-emerald-700/80 dark:text-emerald-400/70 font-medium">
          ...
          <AnimatePresence>
            ...
            {entries.map(entry => (
              <motion.div
                ...
                `}
              >
                <span className="font-mono text-xs font-medium text-emerald-700 dark:text-emerald-300 whitespace-nowrap">
                  {entry.label}
                </span>
              </motion.div>
            ))}
