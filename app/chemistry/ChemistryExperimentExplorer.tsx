...
import { ArrowRight, Search, FlaskConical, Atom, Sparkles } from "lucide-react";

export type ChemistryExperiment = {
  href: string;
  title: string;
  desc: string;
  formula: string;
...
const categories = ["All", "Atomic Structure", "Molecular", "Physical", "Analytical"];
...
function ExperimentCard({ exp }: { exp: ChemistryExperiment }) {
    ...
    <article className="group">
      ...
      >
        ...
        <div>
          ...
          {/* Title */}
          <h3 className="text-xl font-extrabold text-foreground group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors mb-2.5 tracking-tight leading-snug">
            {exp.title}
          </h3>
        ...
        <div className="pt-4 border-t border-border/60 flex items-center justify-between mt-auto">
          <div className="min-w-0 pr-2">
            ...
            <code className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 truncate block">
              {exp.formula}
            </code>
          </div>
...
export default function ChemistryExperimentExplorer({
  experiments,
}: {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-2 rounded-2xl bg-card/80 backdrop-blur-sm border border-border shadow-sm">
        ...
        <div className="relative w-full md:w-96">
          ...
          <input
            type="text"
            placeholder="Search chemistry experiments, reactions, or formulas..."
            value={search}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 bg-card rounded-3xl border border-border p-8">
          ...
          <p className="text-base font-bold text-foreground">No chemistry simulations found</p>
