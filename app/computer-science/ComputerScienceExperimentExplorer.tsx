...
import { ArrowRight, Search, Binary, Code, Cpu } from "lucide-react";

export type CSExperiment = {
  href: string;
  title: string;
  desc: string;
  formula: string;
...
function ExperimentCard({ exp }: { exp: CSExperiment }) {
  ...
  return (
    <article className="group">
      ...
      >
        <div className="absolute top-0 left-0 right-0 h-1 bg-muted group-hover:bg-purple-500/30 transition-all" />
        ...
        <div>
          ...
          {/* Title */}
          <h3 className="text-xl font-extrabold text-foreground group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors mb-2.5 tracking-tight leading-snug">
            {exp.title}
          </h3>
        ...
        <div className="pt-4 border-t border-border/60 flex items-center justify-between mt-auto">
          <div className="min-w-0 pr-2">
            ...
            <code className="text-xs font-mono font-bold text-purple-600 dark:text-purple-400 truncate block">
              {exp.formula}
            </code>
          </div>
...
export default function ComputerScienceExperimentExplorer({
  experiments,
}: {
  experiments: CSExperiment[];
}) {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-2 rounded-2xl bg-card/80 backdrop-blur-sm border border-border shadow-sm">
        ...
        <div className="relative w-full md:w-96">
          ...
          <input
            type="text"
            placeholder="Search CS labs, algorithms, logic gates, AI..."
            value={search}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 bg-card rounded-3xl border border-border p-8">
          <Binary className="w-12 h-12 mx-auto text-muted-foreground/40 mb-3" />
          <p className="text-base font-bold text-foreground">No computer science simulations found</p>
