  ...
  return (
    <main className="min-h-screen bg-[#fafafa] p-6 md:p-12">
      <motion.div initial="hidden" animate="visible" variants={container} className="max-w-6xl mx-auto">
        {title && (
          <div className="mb-10">
            <motion.h1 variants={item} className="text-3xl font-bold tracking-tight text-slate-900">
              {title}
            </motion.h1>
            {description && (
            ...
            <motion.div key={card.href} variants={item} whileHover={{ y: -5 }} whileTap={{ scale: 0.98 }} className="group relative">
              <Link href={card.href} className="relative block h-full">
                ...
                <div className="relative h-full bg-white rounded-xl border border-slate-200 p-6 shadow-[0_2px_4px_rgba(0,0,0,0.02)] group-hover:shadow-xl group-hover:border-slate-300 transition-all duration-300">
                  ...
                  <h3 className="text-xl font-bold text-slate-800 group-hover:text-indigo-600 transition-colors">{card.title}</h3>
                  <p className="text-slate-500 mt-3 leading-relaxed text-sm">{card.desc}</p>
