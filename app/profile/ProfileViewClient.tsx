    ...
    <div className="max-w-6xl mx-auto pb-12 px-4 pt-8">
      ...
      <div className="relative bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden mb-8">
        ...
        <div className="px-4 sm:px-8 pb-8">
          <div className="relative flex flex-col md:flex-row items-center md:items-end gap-6 -mt-16 mb-6 text-center md:text-left">
            <div className="relative">
              <div className="w-32 h-32 rounded-2xl border-4 border-white shadow-lg bg-slate-50 overflow-hidden relative mx-auto md:mx-0">
                <img
            ...
            <div className="flex-1 pb-2 w-full">
              {!editing ? (
                <>
                  ...
                  <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center md:justify-start gap-3 sm:gap-4 mt-3 text-sm text-slate-500">
                ...
                <div className="space-y-4 w-full max-w-md mx-auto md:mx-0">
                  <input
                    className="block w-full text-2xl font-bold border-b-2 border-indigo-200 focus:border-indigo-500 outline-none transition-colors bg-transparent text-slate-900 text-center md:text-left"
                    value={form.username}
            ...
            <div className="flex flex-col sm:flex-row gap-3 pb-2 w-full md:w-auto justify-center">
              {!editing ? (
                <>
                  <button onClick={() => setEditing(true)} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-medium transition-all shadow-sm text-sm">
                    <Edit2 size={16} /> Edit Profile
                  </button>
                  <button onClick={() => fetch('/api/auth/logout', { method: 'POST' }).then(() => router.push('/'))} className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-slate-200 hover:border-rose-200 hover:text-rose-600 text-slate-600 rounded-xl font-medium transition-all shadow-sm text-sm">
                    <LogOut size={16} /> Logout
                  </button>
                </>
              ) : (
                <div className="flex flex-col sm:flex-row gap-2 w-full">
                  <button onClick={handleSave} disabled={saving} className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-semibold shadow-md shadow-indigo-200 transition-all disabled:opacity-50 text-sm">
                    {saving ? 'Saving...' : 'Save Changes'}
                  </button>
                  <button onClick={() => setEditing(false)} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl font-semibold hover:bg-slate-50 transition-all text-sm">
                    Cancel
                  </button>
                </div>
              )}
          ...
          {!editing ? (
            <p className="text-slate-600 max-w-2xl text-lg leading-relaxed mt-2 text-center md:text-left">{user.bio || "No bio yet. Tell the world about your scientific journey!"}</p>
          ) : (
            <div className="space-y-6 mt-4">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block text-center md:text-left">About You</label>
                <textarea
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-3 text-center md:text-left">Choose Your Avatar</label>
            ...
            {(!user.completedExperiments || user.completedExperiments.length === 0) ? (
              ...
              <div className="space-y-3">
                {user.completedExperiments.slice(0, 5).map((exp: any, idx: number) => (
                  <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-indigo-100 hover:bg-indigo-50/50 transition-colors gap-3">
