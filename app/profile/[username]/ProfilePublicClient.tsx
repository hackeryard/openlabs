...
function XPBar({ xp, level, color = "from-indigo-500 to-blue-500" }: { xp: number; level: number; color?: string }) {
  ...
  return (
    <div className="space-y-1.5 w-full">
      <div className="relative w-full bg-muted rounded-full h-3 p-[2px] border border-border shadow-inner overflow-hidden">
        <div
...
export default function ProfilePublicClient({ username }: { username: string }) {
  ...
  return (
    <div className="space-y-8 text-foreground font-sans">
      ...
      {/* Profile Header Card */}
      <div className="relative bg-card rounded-3xl shadow-[0_4px_25px_rgba(0,0,0,0.03)] border border-border overflow-hidden">
        ...
        <div className="px-6 sm:px-8 pb-8 relative z-10">
          <div className="relative flex flex-col md:flex-row items-center md:items-end gap-5 -mt-16 mb-5 text-center md:text-left">
            ...
            <div className="relative">
              <div className="w-28 h-28 rounded-full border-4 border-card shadow-xl bg-muted overflow-hidden relative mx-auto md:mx-0 ring-1 ring-border">
                <img
            ...
            <div className="flex-1 pb-1 space-y-1">
              <div className="flex flex-col md:flex-row md:items-center gap-2 justify-center md:justify-start">
                ...
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-primary/10 text-primary border border-indigo-200 self-center md:self-auto max-w-max mx-auto md:mx-0 shadow-sm">
              ...
              <div className="flex items-center justify-center md:justify-start gap-1.5 pt-1.5 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
          ...
          <div className="border-t border-border pt-4 mt-2">
            <p className="text-muted-foreground max-w-3xl text-sm leading-relaxed font-medium text-center md:text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        ...
        <div className="lg:col-span-2 space-y-6 flex flex-col">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <motion.div 
              whileHover={{ y: -3 }}
              className="bg-card border border-border rounded-2xl p-4 flex items-center gap-3.5 transition-all hover:border-indigo-200 hover:shadow-lg hover:shadow-indigo-500/5 select-none relative overflow-hidden shadow-sm"
            >
              ...
              <div>
                <p className="text-[9px] font-extrabold uppercase tracking-wider text-muted-foreground">Level</p>
                <h4 className="text-xl font-black text-foreground leading-tight mt-0.5">{user.level}</h4>
              </div>
            </motion.div>
            ...
            <motion.div 
              whileHover={{ y: -3 }}
              className="bg-card border border-border rounded-2xl p-4 flex items-center gap-3.5 transition-all hover:border-orange-200 hover:shadow-lg hover:shadow-orange-500/5 select-none relative overflow-hidden shadow-sm"
            >
              ...
              <div>
                <p className="text-[9px] font-extrabold uppercase tracking-wider text-muted-foreground">Streak</p>
                <h4 className="text-xl font-black text-foreground leading-tight mt-0.5">{user.streak}</h4>
              </div>
            </motion.div>
            ...
            <motion.div 
              whileHover={{ y: -3 }}
              className="bg-card border border-border rounded-2xl p-4 flex items-center gap-3.5 transition-all hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-500/5 select-none relative overflow-hidden shadow-sm"
            >
              ...
              <div>
                <p className="text-[9px] font-extrabold uppercase tracking-wider text-muted-foreground">Labs Done</p>
                <h4 className="text-xl font-black text-foreground leading-tight mt-0.5">{totalCompleted}</h4>
              </div>
            </motion.div>
            ...
            <motion.div 
              whileHover={{ y: -3 }}
              className="bg-card border border-border rounded-2xl p-4 flex items-center gap-3.5 transition-all hover:border-blue-200 hover:shadow-lg hover:shadow-blue-500/5 select-none relative overflow-hidden shadow-sm"
            >
              ...
              <div>
                <p className="text-[9px] font-extrabold uppercase tracking-wider text-muted-foreground">Total XP</p>
                <h4 className="text-xl font-black text-foreground leading-tight mt-0.5">{user.xp}</h4>
              </div>
            </motion.div>
          </div>

          {/* Subject Mastery Panel */}
          <div className="bg-card p-6 rounded-3xl border border-border shadow-[0_4px_25px_rgba(0,0,0,0.03)] space-y-6 flex-grow">
            ...
            {(!user.subjectProgress || user.subjectProgress.length === 0) ? (
              <div className="text-center py-10 text-muted-foreground bg-muted rounded-2xl border border-border border-dashed text-xs font-semibold">
        ...
        <div className="flex flex-col">
          <div className="bg-card p-6 rounded-3xl border border-border shadow-[0_4px_25px_rgba(0,0,0,0.03)] relative overflow-hidden flex flex-col h-full min-h-[350px]">
            ...
            <div className="relative z-10 flex flex-col h-full space-y-6">
              ...
              {(!user.badges || user.badges.length === 0) ? (
                <div className="flex-grow flex flex-col items-center justify-center text-center py-8 space-y-3">
                  <div className="w-14 h-14 rounded-full bg-muted border border-border flex items-center justify-center text-muted-foreground">
                    <Trophy size={24} className="opacity-40" />
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs font-bold text-muted-foreground">No Achievements Displayed</p>
                  </div>
                </div>
              ) : (
                  ...
                  {user.badges.map((b: any) => {
                    ...
                    return (
                      <motion.div 
                        whileHover={{ y: -2 }}
                        key={b.id} 
                        className="p-3 rounded-2xl bg-card border border-border flex flex-col items-center justify-center text-center cursor-default group"
                      >
                        <div 
                        ...
                        <span className="text-[9px] font-black text-foreground uppercase tracking-wider leading-snug">
                          {b.name}
                        </span>
                        {b.earnedAt && (
                          <span className="text-[8px] text-muted-foreground font-bold uppercase mt-1">
